...
public:
	...
	//Octomap
	void GetImages(cv::Mat& RGB, cv::Mat& DEPTH);
	cv::Mat mImDep;
	cv::Mat mImRGB;	
    ...
    vector<cv::Rect_<float>> mvDynamicArea;

    std::vector<Object> mvObject; // 2d detection results static
	std::vector<Object> mvDyObject; // 2d detection results dynamic

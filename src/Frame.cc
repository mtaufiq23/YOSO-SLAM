...
Frame::Frame(Tracking* pTracker, const cv::Mat &imGray, const cv::Mat &imDepth, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera* pCamera,Frame* pPrevF, const IMU::Calib &ImuCalib)
    :mpTracker(pTracker),mpcpi(NULL),mpORBvocabulary(voc),mpORBextractorLeft(extractor),mpORBextractorRight(static_cast<ORBextractor*>(NULL)),
     ...
     mpCamera(pCamera),mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    ...
    //YOLO
	if(mpTracker->mpSystem->Yolo_Status==1)
	{
		if(!mpTracker->mpDetector->Objects.empty()) 
		    mbHaveDynamicObjectForMapping = mpTracker->mpDetector->mbHaveDynamicObjectForMapping;

		if(mbHaveDynamicObjectForMapping)
		    mvPotentialDynamicBorderForMapping = mpTracker->mpDetector->mvPotentialDynamicBorderForMapping;
	}
...
void Frame::AssignFeaturesToGrid()
{
	...
	//Each small grid container becomes a sub-container with a number of key points, which can be dynamically resized.
    for(int i=0;i<N;i++)
    {

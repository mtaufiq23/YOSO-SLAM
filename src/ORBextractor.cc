...
#undef GET_VALUE
    ...
    ORBextractor::ORBextractor(int _nfeatures, float _scaleFactor, int _nlevels,
                               int _iniThFAST, int _minThFAST,  const string &strSettingPath):
            ...
            iniThFAST(_iniThFAST), minThFAST(_minThFAST)
    {
		...
		yoloRun = fsSettings["YoloDetect.Run"];	//bool to trigger yolo detection.
        ...
        for (int level = 0; level < nlevels; ++level)
        {
            ...
            for(int i=0; i<nRows; i++)
            {
                ...
                for(int j=0; j<nCols; j++)
                {
                    ...
                    //(Delete features from the preset dynamic object stated in Yolo_Data file)
					if (yoloRun == 1)
					{

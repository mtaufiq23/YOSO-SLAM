...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
    ...
    Objects(F.Objects),mbHaveDynamicObjectForMapping(F.mbHaveDynamicObjectForMapping),
    mvPotentialDynamicBorderForMapping(F.mvPotentialDynamicBorderForMapping)
{
    mnId=nNextId++;
...
cv::Mat KeyFrame::_GetPose()
{
    unique_lock<mutex> lock(mMutexPose);
    return SE3ToCvMat(mTcw);
}

//convert SE3 to cvmat
template <class Scalar>
cv::Mat KeyFrame::SE3ToCvMat(const Sophus::SE3<Scalar>& se3) 
{
...
void KeyFrame::GetImages(cv::Mat &RGB, cv::Mat& DEPTH)
{
    mImRGB = RGB;
	mImDep = DEPTH;
}

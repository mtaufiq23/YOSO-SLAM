...
LoopClosing::LoopClosing(System *pSys, Atlas *pAtlas, KeyFrameDatabase *pDB, ORBVocabulary *pVoc, const bool bFixScale, const bool bActiveLC, MapDrawer *pMapDrawer):
            ...
            if(bFindedRegion)
            {
                if(mbMergeDetected)
                {
                    ...
                    else
                    {
						...
						//Merge Octomap
						if (mpMapDrawer->octo_m_enable==1 && mpViewer->Octomap_Status==1 && (mpTracker->mSensor==System::RGBD || mpTracker->mSensor==System::IMU_RGBD)) 
						mpMapDrawer->octo_mergebusy = true;
						
						//Merge octomap				
						if (mpMapDrawer->octo_m_enable==1 && mpViewer->Octomap_Status==1  && (mpTracker->mSensor==System::RGBD || mpTracker->mSensor==System::IMU_RGBD))
						{
							octo_mergecount++;
							mpCurrentKF->UpdateConnections();	// Ensure current keyframe is updated
							while (mpMapDrawer->octobusy){cout<<"Waiting to clear current octomap..."<<endl;}	//Wait till octomap not busy
							mpMapDrawer->ClearOctomap();	//Clear current octomap & started a new one	
//							mpAtlas->isCreatedMap = true;	//Create new octomap	
						    mpTracker->SetStepByStep(true);			

							while (octo_mergecount!=mpMapDrawer->octo_mergecount) 
							{
								mpMapDrawer->lockocto2 = true;	
								mpMapDrawer->currentvpKFs = mpAtlas->GetAllKeyFrames();//  Get all keyframes=====	
								mpMapDrawer->MergeUpdate();
							}
							cout<<"After mpMapDrawer->octo_mergecount = " << mpMapDrawer->octo_mergecount <<endl;
                                                 ...
                                                 std::vector<MapPoint*> &vpMPs, std::vector<MapPoint*> &vpMatchedMPs)
{
    ...
    if(nNumProjMatches >= nProjMatches)
    {
        ...
        {
//            g2o::Sim3 gScw_estimation(Converter::toMatrix3d(mscw.rowRange(0, 3).colRange((0,3)), Converter::toVector3d(mscw.rowRange(0, 3).col(3))1.0);
...
{  
    ...
    // We need to propagate the correction through the spanning tree
    {
        ...
        if(!mbStopGBA)
        {
			...
			//loop octo
			if (mpMapDrawer->octo_lc_enable==1 && mpViewer->Octomap_Status==1 && (mpTracker->mSensor==System::RGBD || mpTracker->mSensor==System::IMU_RGBD)) 
				mpMapDrawer->octo_loopbusy = true;
...
//				mpAtlas->isCreatedMap = true;	//Create new octomap
				...
				while (octo_loopcount!=mpMapDrawer->octo_loopcount) 
				{	
					mpMapDrawer->lockocto = true;
					mpMapDrawer->currentvpKFs = mpAtlas->GetAllKeyFrames();//  Get all keyframes=====		
					mpMapDrawer->LoopUpdate();
				}
				cout<<"After mpMapDrawer->octo_loopcount = " << mpMapDrawer->octo_loopcount <<endl;
				mpTracker->SetStepByStep(false);
			}

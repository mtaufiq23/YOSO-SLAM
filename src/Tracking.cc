    ...
    mnInitialFrameId(0), mbCreatedMap(false), mnFirstFrameId(0), mpCamera2(nullptr), mpLastKeyFrame(static_cast<KeyFrame*>(NULL))
{
    ...
    if(settings){
        ...
        // Load ORB parameters
        bool b_parse_orb = ParseORBParamFile(fSettings, strSettingPath);
        if(!b_parse_orb)
        {
    ...
    if (mpSystem->Yolo_Status == 1) std::cout << "Object detection using 'Yolov8' & dynamic point detection with bounding box." << std::endl;
    else std::cout << "'No' Dynamic point detection used" << std::endl;
	
	if (mpMapDrawer->octodrawobj == 1) std::cout << "Semantic 3D Map: Draw both Octomap & objects." << std::endl;
    else std::cout << "<3D Map: Draw Octomap only." << std::endl;
...
void Tracking::newParameterLoader(Settings *settings, const string &strSettingPath) {
    ...
    mpORBextractorLeft = new ORBextractor(nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST,strSettingPath);

    if(mSensor==System::STEREO || mSensor==System::IMU_STEREO)
        mpORBextractorRight = new ORBextractor(nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST,strSettingPath);

    if(mSensor==System::MONOCULAR || mSensor==System::IMU_MONOCULAR)
        mpIniORBextractor = new ORBextractor(5*nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST,strSettingPath);
...
bool Tracking::ParseORBParamFile(cv::FileStorage &fSettings, const string &strSettingPath)
{
    bool b_miss_params = false;
...
void Tracking::KFDetector(KeyFrame* tkf)
{
    mptkf = tkf;
}
...
Sophus::SE3f Tracking::GrabImageRGBD1(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp, string filename)
{
    mImGray = imRGB;
    cv::Mat imDepth = imD;
    ...
    if(mpDetector->mvKFno.size()>0)
	{
	   if(mpDetector->Objects.size()>0)
       {  
           std::cout << "Detect : " << mpDetector->Objects.size() << " static objects" << std::endl;
           for(unsigned int j =0; j<mpDetector->Objects.size(); j++)
           {
               unique_lock<mutex> lckObj(mvKeyframesMutex); // 2d detection results locked
               mpDetector->mvKeyframes[mpDetector->mvKFno.front()]->mvObject.push_back(mpDetector->Objects[j]);// Store target detection results====
           }
       }
	   if(mpDetector->DyObjects.size()>0)
       {  
           std::cout << "Detect : " << mpDetector->DyObjects.size() << " dynamic objects" << std::endl;
           for(unsigned int j =0; j<mpDetector->DyObjects.size(); j++)
           {
               unique_lock<mutex> lckObj(mvKeyframesMutex); // 2d detection results locked
               mpDetector->mvKeyframes[mpDetector->mvKFno.front()]->mvDyObject.push_back(mpDetector->DyObjects[j]);// Store target detection results====
           }
       }
	   mpDetector->mvKFno.pop();
	}
    ...
    //Yolo
	if (mpSystem->Yolo_Status==1){	//Dynamic point collection using yolo bounding box area
...
void Tracking::Track()
{
    ...
    else
    {
        ...
        if(bOK || mState==RECENTLY_LOST)
        {
			...
			if (mpMapDrawer != NULL) mpMapDrawer->SetCurrentCameraPose(mCurrentFrame.GetPose()); //====new===
        ...
        if(mState==LOST)
        {
			...
			// Force save current octomap when new map created ==============
			if(mpViewer->Octomap_Status==1) 
			{
				if(mSensor == mpSystem->RGBD)
				{
					mpMapDrawer->SaveOctoMap(mpViewer->octobaksavepath);// Save octomap map
					cout<<"Save backup current Octomap(RGBD) done!"<<endl;
				}				
...
void Tracking::CreateMapInAtlas()
{
	...
	//		//Octomap
//		if (mpViewer->Octomap_Status==1 && (mSensor == System::RGBD || mSensor == System::IMU_RGBD)) 
//			mpMapDrawer->ClearOctomap();	//Clear current octomap & started a new one
...
//    KeyFrame* pKF = new KeyFrame(mCurrentFrame,mpAtlas->GetCurrentMap(),mpKeyFrameDB);
	...
	//Octomap
	KeyFrame* pKF;
	if(mSensor == System::RGBD || mSensor == System::IMU_RGBD)// rgbd or rgbd-inertial	    
	    pKF = new KeyFrame(mCurrentFrame,mpAtlas->GetCurrentMap(),mpKeyFrameDB, this->mImRGB, this->mImDepth);
    else
	...
	if (mpSystem->Yolo_Status==1) 
	{
		mpDetector->InsertKeyFrame(pKF);
		pKF->mvDynamicArea = mCurrentFrame.mvDynamicArea;
	}
	else if (mpSystem->Yolo_Status==0){}
	else{
		std::cout << "Error: Undefined YoloDetect.Run value! Use 0 or 1. Exiting.." << std::endl;
		exit(0);
	}   
	...
	if (mpViewer->Octomap_Status==1){
    	if (mSensor == System::RGBD || mSensor == System::IMU_RGBD) 
		{
			if (!bStepByStep) 
			{
				...
				mpMapDrawer->InsertKeyFrame1(pKF, this->mImRGB, this->mImDepth, idk, vpKFs);	//process filtering during keyframe received
			}
		}
	}
	else if (mpViewer->Octomap_Status==0){}
	else{
...
// Add whether to record relocation records================================
bool Tracking::Relocalization(bool save_changed)
{
    Verbose::PrintMess("Starting relocalization", Verbose::VERBOSITY_NORMAL);
    // Compute Bag of Words Vector
    mCurrentFrame.ComputeBoW();
    ...
    {
		if(save_changed) mnLastRelocFrameId = mCurrentFrame.mnId;
        cout << "Relocalized!!" << endl;
        return true;
    }

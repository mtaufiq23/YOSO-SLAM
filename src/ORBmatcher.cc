...
//	// Project a frame to search for matching points and return 2d-2d matching point pairs.============================================
...
//	    // Rotation Histogram (to check rotation consistency)
//	   // Matching point observation direction difference histogram statistics are used to filter the best matches
//	    vector<int> rotHist[HISTO_LENGTH];
//	    for(int i=0;i<HISTO_LENGTH;i++)
//		rotHist[i].reserve(500);
//	    const float factor = 1.0f/HISTO_LENGTH;

//	 	cv::Mat temp = CurrentFrame._mTcw;
//		temp = ORB_SLAM3::Converter::SE3ToCvMat(CurrentFrame.mTcw);	
//	 // Current frame rotation translation matrix   
//	    const cv::Mat Rcw = temp.rowRange(0,3).colRange(0,3);
//	    const cv::Mat tcw = temp.rowRange(0,3).col(3);
//	    const cv::Mat twc = -Rcw.t()*tcw;// // twc(w)

////		const Sophus::SE3f tcw = CurrentFrame.GetPose();
////        const Eigen::Vector3f twc = tcw.inverse().translation();
//	
//		cv::Mat temp_ = LastFrame._mTcw;
//		temp_ = ORB_SLAM3::Converter::SE3ToCvMat(LastFrame.mTcw);	
//	// Previous frame rotation translation matrix
//	    const cv::Mat Rlw = temp_.rowRange(0,3).colRange(0,3);
//	    const cv::Mat tlw = temp_.rowRange(0,3).col(3);
//	    const cv::Mat tlc = Rlw*twc + tlw;//Translation vector from current frame to previous frame
...
////		const bool bForward = tlc(2)>CurrentFrame.mb && !bMono;
//	     // In non-monocular situations, if Z>0 and greater than the baseline, it means moving forward
//	    
//	    const bool bBackward = -tlc.at<float>(2) > CurrentFrame.mb && !bMono;
////		const bool bBackward = -tlc(2)>CurrentFrame.mb && !bMono;
//	    // In non-monocular situations, if Z<0 and the absolute value is greater than the baseline, it means moving forward.
//	    
...
//		{
//		    if(!LastFrame.mvbOutlier[i])// This map point is not an exterior point, but an interior point, a point with a compound transformation relationship.
//		    {
//			// Project
//// Step 3: Project the map points of the previous frame onto the pixel plane of the current frame-----------------------------------------    
//		    Eigen::Vector3f P = pMP->GetWorldPos();// Previous frame map point (under world coordinate system);
////			Eigen::Matrix4f M;
////			M.row(0) = P;
//			cv::Mat x3Dw;
//            x3Dw = ORB_SLAM3::Converter::toCvMat(P);
////			cv::eigen2cv(P, x3Dw);
//			cv::Mat x3Dc = Rcw*x3Dw+tcw;//Previous frame map point (under the current frame coordinate system)
...
//			    continue;// Need to be within image dimensions
...
//// When moving forward, the area of ​​the dot increases. It is a feature point at a certain scale m. Because the area increases, it needs to be detected at a higher scale.
//// Therefore m>=n, corresponding to the forward situation, nCurOctave>=nLastOctave. The situation of retreat can be analogized
//			int nLastOctave = LastFrame.mvKeys[i].octave;//  The scale of the map point in the previous frame corresponding to the feature point (number of pyramid levels)
//			// Search in a window. Size depends on scale
//			float radius = th * CurrentFrame.mvScaleFactors[nLastOctave];//The larger the scale, the larger the search range
//			vector<size_t> vIndices2;// Candidate points near the projection point on the current frame
//			if(bForward)// Moving forward, the interest point of the previous frame is at the scale nLastOctave <= nCurOctave< 8 (it is closer, the scale is large, and the 
//						//number of layers is high, it can also be seen)
//			    vIndices2 = CurrentFrame.GetFeaturesInArea(u,v, radius, nLastOctave);
//			else if(bBackward)// Go back, then the scale of the previous frame's interest point is 0 <= nCurOctave <= nLastOctave (the scale is reduced when it is far away)
//			    vIndices2 = CurrentFrame.GetFeaturesInArea(u,v, radius, 0, nLastOctave);
//			else// Not much movement. In the previous frame, scale additional search.
//			    vIndices2 = CurrentFrame.GetFeaturesInArea(u,v, radius, nLastOctave-1, nLastOctave+1);
//			if(vIndices2.empty())
//			    continue;
//			
//// Step 5: Traverse the candidate key points, calculate the match with the map point descriptor, calculate the distance, and keep the closest match
//			const cv::Mat dMP = pMP->GetDescriptor();// Previous frame map point descriptor
//			int bestDist = 256;
//			int bestIdx2 = -1;	
...
//			{
//			    const size_t i2 = *vit;
//			    if(CurrentFrame.mvpMapPoints[i2])// If the current frame keyframe has map points
//				if(CurrentFrame.mvpMapPoints[i2]->Observations() > 0)//The corresponding map point also has an observation frame, so skip it.
//				    continue;//Skip unused map points when matching
//				    
//                           // In the case of binocular and rgbd, you need to ensure that the points on the right are also within the search radius.
//			    if(CurrentFrame.mvuRight[i2]>0)
//			    {
//				const float ur = u - CurrentFrame.mbf*invzc;//Matching point The abscissa of the right image
//				const float er = fabs(ur - CurrentFrame.mvuRight[i2]);// error
//				if(er > radius)
//				    continue;
//			    }

//			    const cv::Mat &d = CurrentFrame.mDescriptors.row(i2);// Current frame keypoint descriptor
//			    const int dist = DescriptorDistance(dMP,d);// Descriptor matching distance
//			    if(dist<bestDist)
//			    {
//				bestDist=dist;//shortest distance
//				bestIdx2=i2;// Corresponding key point subscript of the current frame
//			    }
//			}

//			if(bestDist<=TH_HIGH)// The shortest distance is less than <100
//			{
//			    CurrentFrame.mvpMapPoints[bestIdx2]=pMP;// Match the keypoints of the current frame to the map points of the previous frame
//			    nmatches++;
//// ============================================new=================
//                            points_last.push_back( LastFrame.mvKeys[i].pt );// Key points of the previous frame
//                            points_current.push_back( CurrentFrame.mvKeys[bestIdx2].pt );// Current frame keypoint====
//                            
//                         // Matching points Observation direction difference Consistency detection
...
//				    bin=0;
//				assert(bin>=0 && bin<HISTO_LENGTH);
//				rotHist[bin].push_back(bestIdx2);//Statistics to the corresponding direction histogram
//			    }
//			}
//		    }
//		}
//	    }
//// // Step 6: Eliminate mismatched points based on direction difference consistency constraints
//	    //Apply rotation consistency
//	    if(mbCheckOrientation)
//	    {
//		int ind1=-1;
...
//// In addition, if the highest one is more than 10 times higher than the second highest one, only the matching points in the highest bin will be retained.
//// If the highest one is more than 10 times higher than the third highest one, keep the matching points in the highest and second highest bins.
//		ComputeThreeMaxima(rotHist,HISTO_LENGTH,ind1,ind2,ind3);
...
//                            points_current.push_back( CurrentFrame.mvKeys[bestIdx2].pt );// Current frame keypoint====
					    ...
					    // ============================================new===================================================
                        points_last.push_back( kpLF.pt );// Key points of the previous frame
                        points_current.push_back( kpCF.pt );// Current frame keypoint====

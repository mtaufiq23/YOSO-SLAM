    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    cout << endl <<
    "################################################################################" << endl <<
    "# YOSO-SLAM: A Real-time Object Visual SLAM for Dynamic                        #" << endl <<                   
    "#            Scenes with Semantic 3-Dimensional Mapping                        #" << endl << 
    "################################################################################" << endl << endl;
...
//    		 pclFinish = false;
	...
	//yolo setting
	Yolo_Status = fsSettings["YoloDetect.Run"];
...
//    mpFrameDrawer = new FrameDrawer(mpAtlas);
	mpFrameDrawer = new FrameDrawer(mpAtlas, mpMapDrawer, strSettingsFile);	//Keyframe display
    mpMapDrawer = new MapDrawer(mpAtlas, strSettingsFile, settings_, mpDetector);	//Map display
    ...
    //Yolo
	if (Yolo_Status==1) mpTracker->SetDetector(mpDetector);
    ...
    mpLoopCloser = new LoopClosing(this, mpAtlas, mpKeyFrameDatabase, mpVocabulary, mSensor!=MONOCULAR, activeLC, mpMapDrawer); // mSensor!=MONOCULAR);
...
Sophus::SE3f System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    Tcw = mpTracker->GrabImageRGBD1(imToFeed,imDepthToFeed,timestamp,filename);	//Starting YOSO-SLAM
...
void System::Shutdown()
{
    ...
    cout << "Preparing to shutdown" << endl;
	...
	if(mpViewer)
    {
        ...
        {
            usleep(5000);
            cout << "Viewer not finish yet" << endl;
        }
    }	
...
// Save both sparse map (not working atm ~ use save atlas instead)
...
// Save octomap
void System::OctoMapSave(const std::string& filename)
{
    if (mpViewer->Octomap_Status==1)
	{
		mpMapDrawer->SaveOctoMap(filename);
		cout<<"日 Save Octomap(RGBD) done!"<<endl;
	}
}


void System::SaveAtlas2(int type, const string &mStrSaveAtlasToFile)
{
    if(!mStrSaveAtlasToFile.empty())
    {
        //clock_t start = clock();
